...
4b. UNACKNOWLEDGED REQUESTS — most critical category. List every client message in the chat window that contains a request, a question, or a decision the team has to make, where there is no team reply within 24 hours of the client message. For each entry: "request" = a one-line paraphrase, "verbatim" = the EXACT quote from the client's message (copy verbatim, do not paraphrase), "asked_by" = client display label, "asked_on" = DD MMM, "days_unanswered" = whole days from the client message to TODAY. Return an empty array ONLY after you have scanned every client-group message in the window and confirmed each one received a substantive (non-deflection) team reply OR a client self-confirmation. Emptying this section because it was hard to populate, or skipping the scan, is the single worst failure this brief can make. Do NOT skip entries because they seem minor — an unanswered client is the highest-priority signal. A "request" is also when the client provides a piece of information that needs an acknowledgement, not just an explicit question. CRITICAL — DEFLECTION IS NOT AN ANSWER: a team reply like "we are working on it", "waiting for vendor confirmation", "will share soon", "let me check and revert", or any holding response that does NOT actually deliver what the client asked for, does NOT close the unack entry. The request stays in the unack list until the actual thing the client asked for is delivered (profile shared, call time confirmed, decision made, document sent). MIRROR RULE: if you wrote a needs_you item containing phrases like "client waiting", "unanswered Nd", "request from [client] not addressed", or "[client] asked X but no response" — the SAME issue MUST also appear as an unacknowledged_requests entry. needs_you and unacknowledged_requests are NOT alternatives — needs_you tells Amaan what to do; unack lists what the client is actively waiting on. The same client-side ask appears in BOTH sections. Use the client's first name (e.g. "Dharmik") as asked_by — do NOT include role suffix or partner names. NOT-AN-UNACK: a thread is closed if the CLIENT's own later message accepts or confirms it ("yes works for us", "sounds good", "ok confirmed", "perfect") — even with no separate team reply. Do not list a confirmed item as unacknowledged. When a scheduling/decision thread is resolved but has no visible MOM, call link, or text trail, that is a needs_you item with risk_type: process or visibility — NOT an unacknowledged_request. NEVER list as an unacknowledged_request: (i) a TEAM→client ask (the team asking the client to send/confirm/decide — the client is not the one waiting; that is a commitment or a needs_you, not an unack); (ii) a client PROMISE the client made ("I'll pay tomorrow", "we'll send the image") — that is a client commitment, not something the client is waiting on; (iii) anything said only in the INTERNAL group — unack is CLIENT-group messages only; (iv) a question the team already answered substantively or the client already confirmed. THE ONE TEST: a client, in the client group, asked for something or raised something needing a reply, and the team did NOT substantively deliver it. If it fails that test, it is not an unack.
...
6. CROSS-SOURCE FLAGS: raise a flag when chat clearly contradicts a tracker field, OR when commercial vocabulary (CP, SP, markup, margin, commission) appears in the CLIENT group. ALSO raise a cross_source_flag — these are severity-1 trust/commercial risks that belong here FIRST, not only in needs_you — when: (a) the client obtained or cites a vendor-DIRECT price and compares it to ours (markup / pricing-wall exposure, e.g. "the photographer quoted us ₹4.5L directly vs your ₹6L"); (b) a price-EXPECTATION gap is visible (the client was told or expects one figure but the real quote differs materially); (c) the client signals intent to source a vendor themselves (self-sourcing — also see WS41). Capture the operational consequence in chat_says vs tracker_says (what the client now believes/obtained vs what we quoted or recorded). You may also mention it in needs_you, but it must NOT live ONLY in needs_you. Do not flag speculative differences.
7. NEEDS YOU: surface only things that genuinely require Amaan's decision or action. Each item has a "headline" (<15 words, TL-scannable) and a "detail" (full explanation with attribution). Tag each with a "risk_type" from: sentiment, collection, visibility, process, execution, cancellation. Order client-experienced flags first. POST-ESCALATION WEIGHTING: if the chat or a deterministic flag shows a prior client complaint about communication, a past escalation, or a recovery situation, then missing MOMs, absent text trails, unconfirmed call outcomes, and profiles stuck internally each jump UP in needs_you priority — in a recovering relationship, every dropped trail is a fresh trust risk, not housekeeping. In a recovery or post-complaint situation specifically, a missing MOM or absent text-trail after a call is a MANDATORY needs_you item (risk_type: visibility) — surface it explicitly, do not merely reprioritize or assume the call going well is enough.
...
21. GROUNDING DISCIPLINE — do not manufacture specifics. (a) Never state a vendor price, sum, count, or figure unless that exact number appears in the signals or a TRACKER field — do not round, estimate, or "remember" a number from elsewhere. (b) Never assert that something was "shared", "sent", "confirmed", or "delivered" unless a message in the window shows it happening; if you are unsure whether it happened, treat it as OPEN, not done. (c) Never attach a named person to a message whose sender shows as Unknown, "?", or a raw @lid string (reinforces rule 2) — being the known TL/planner is not licence to guess who an unknown sender is. (d) Genuine inferences and "I think but can't confirm" reads belong in ai_clarification, never asserted as fact in summaries. This rule targets invented specifics ONLY — it does not discourage well-supported analytical inference (e.g. spotting a tracker-vs-chat contradiction), which remains a priority.
22. INTERNAL-SOURCED IS NOT CLIENT-DELIVERED. A vendor profile, quote, document, or option that appears ONLY in the internal group has NOT reached the client. Do not describe it as "shared with the client". This is an open execution gap: the team has the thing but the client is still waiting. Surface it in needs_you (risk_type: execution or visibility) and, if the client previously asked for it, mirror it into unacknowledged_requests. The operationally useful read is always "sourced internally" vs "delivered to client" — keep them distinct.
23. CALIBRATE CERTAINTY AND PRIORITY. (a) Do not assert a decision as final ("selected", "confirmed", "booked", "resolved", "finalised") unless the CLIENT explicitly chose it or the team explicitly executed it in the window. If the client only acknowledged, showed interest, joined a call, or said "will revert", it is still PENDING — record it in decision_intel.pending_decisions, not as a recent_decision or a closed item. (b) priority "urgent" is reserved for active client friction (a visible complaint, an ask the client is actively chasing, or a sentiment / cancellation / self-sourcing signal) OR an imminent deadline (within ~7 days). Do NOT mark far-out collection on an early-runway PID "urgent" — low collection at T-150+ with no client friction is "soon" or "when_able". (c) client_pulse.confidence must follow the message-count scale (high = 20+ client messages, medium = 5-20, low = under 5) — do not inflate confidence beyond what the volume supports.
...
## Final self-audit (run silently before emitting JSON — do not include in output)
Before you return the JSON, verify each of these. If any fails, fix it, then emit:
1. what_changed has ≥3 items whenever the signal count is non-zero. An empty or placeholder what_changed with signals present is a FAILURE.
2. You scanned EVERY client-group message in the window for an unanswered request, question, or piece of info needing acknowledgement. unacknowledged_requests is empty ONLY if you can name no such message — not because the section was hard.
3. Every number, sum, price, "shared/sent/confirmed" status, and named sender in your output appears literally in the signals or a TRACKER field. If you cannot point to where it came from, remove it or move the uncertainty into ai_clarification.
4. Every needs_you item that says a client is waiting also appears in unacknowledged_requests (the mirror rule), and vice-versa.
5. Each recent_decision's decided_by is the person who actually decided — a team plan-to-do is NOT a client decision.
6. Any client comparison to a vendor-direct price, any price-expectation gap, and any self-sourcing signal appears in cross_source_flags — not only in needs_you. If you wrote about pricing exposure or self-sourcing anywhere, there is a matching cross_source_flags entry.
7. Every unacknowledged_requests entry passes THE ONE TEST: a client, in the client group, asked for something the team did not substantively deliver. None are team→client asks, client promises, internal-group messages, or already-answered items.
8. No decision is marked final/selected/confirmed unless the client chose or the team executed it; nothing far-out and frictionless is marked "urgent"; client_pulse.confidence matches the client-message count.`;
...
async function withRetry<T>(fn: () => Promise<T>, retries = 3, baseDelayMs = 3000): Promise<T> {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // Transient API errors (rate/overload) AND connection drops. DeepSeek closes the
      // socket ("terminated") on long generations (e.g. PID 20614 ~276s) — without this,
      // the biggest-prompt PIDs silently skip their nightly brief and go stale.
      const isRetryable =
